...
const PREFERENCES = [
  ...
  { preference_id: 19, staff_id: 6, course_id: 2, preference_rank: 1 },
  { preference_id: 20, staff_id: 6, course_id: 4, preference_rank: 2 },

  { preference_id: 21, staff_id: 7, course_id: 1, preference_rank: 1 },
  { preference_id: 22, staff_id: 7, course_id: 6, preference_rank: 2 },

  { preference_id: 23, staff_id: 8, course_id: 5, preference_rank: 1 },
  { preference_id: 24, staff_id: 8, course_id: 8, preference_rank: 2 },

  { preference_id: 25, staff_id: 9, course_id: 3, preference_rank: 1 },
  { preference_id: 26, staff_id: 9, course_id: 7, preference_rank: 2 },

  { preference_id: 27, staff_id: 10, course_id: 6, preference_rank: 1 },
...
export default function AdminDashboardProf() {
  ...
  const getPreferencesByRank = (staffId, rank) => {
    return preferences.filter(
      (p) => p.staff_id === staffId && p.preference_rank === rank,
    );
  };
  ...
  const validateConstraints = (staffId, courseIds) => {
    ...
    if (stf.is_senior) {
      const labCourses = courseIds
        .map((id) => getCourseById(id))
        .filter((c) => c.is_lab_associated);
      if (labCourses.length > 1) {
        errors.push(
          " Senior faculty cannot select multiple lab-associated courses",
        );
      }
    }
  ...
  const handleApprove = () => {
    ...
    if (currentAssignments.length > 0) {
      setStaffDetails((prev) =>
        prev.map((d) =>
          d.staff_id === selectedStaffId? { ...d, currentLoad: validation.clh }: d,
        ),
      );
  ...
  const canApprove =
    currentAssignments.length > 0 && validation?.errors.length === 0;
    ...
    <div className="p-5 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-5 h-[80vh]">
        ...
        <div className="overflow-y-auto">
          <h3 className="text-xl font-semibold mb-4 sticky top-0 bg-white z-10 py-2">
            Faculty Preferences
          </h3>
          <div className="bg-gray-50 rounded-lg overflow-hidden">
            {sortedStaff.map((stf, idx) => {
              const detail = staffDetails.find(
                (d) => d.staff_id === stf.staff_id,
              );
              return (
                <div
                  key={stf.staff_id}
                  onClick={() =>
                    setSelectedStaffId(
                      selectedStaffId === stf.staff_id ? null : stf.staff_id,
                    )
                  }
                ...
                >
                  ...
                  <p className="text-sm text-gray-600">
                    {stf.rank} {stf.is_senior ? "(Senior)" : ""} | Load:{" "}
                    {detail?.currentLoad || 0}/{stf.max_load_clh} CLH
                  </p>
                </div>
              );
            })}
        ...
        {/* Assignment Panel - Fixed Size */}
        <div className="h-full overflow-hidden">
          {currentStaffRecord && currentStaffDetail ? (
            <div className="bg-gray-50 rounded-lg h-full flex flex-col">
              <div className="p-4 bg-blue-500 text-white flex-shrink-0">
                <h3 className="text-xl font-bold mb-2">
                  {currentStaffDetail.name}
                </h3>
                <p className="text-sm mb-1">{currentStaffRecord.rank}</p>
                <p className="text-sm mb-2">
                  Current Load: {currentStaffDetail.currentLoad}/
                  {currentStaffRecord.max_load_clh} CLH
                </p>
                {currentAssignments.length > 0 && (
                  <p className="text-sm bg-red-500 bg-opacity-20 p-2 rounded">
                    New Assignment: +{calculateCLH(currentAssignments)} CLH →
                    Total:{" "}
                    {currentStaffDetail.currentLoad +
                      calculateCLH(currentAssignments)}
                    /{currentStaffRecord.max_load_clh} CLH
                  </p>
                )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                ...
                {[1, 2, 3].map((rank) => {
                  const rankedPreferences = getPreferencesByRank(
                    selectedStaffId,
                    rank,
                  );
                    ...
                    <div key={rank} className="mb-4">
                      ...
                      <div className="ml-2 flex flex-wrap gap-2">
                        {rankedPreferences.map((pref) => {
                          ...
                          const isSelected = currentAssignments.includes(
                            pref.course_id,
                          );
                          return (
                            <button
                              ...
                              onClick={() =>
                                isSelected
                                  ? handleUnassignCourse(pref.course_id)
                                  : handleAssignCourse(pref.course_id)
                              }
              ...
              <div className="p-4 border-t border-gray-300 flex gap-2.5 flex-shrink-0">
                <button
            ...
            <div className="bg-gray-50 rounded-lg p-8 h-full flex items-center justify-center text-gray-500">
